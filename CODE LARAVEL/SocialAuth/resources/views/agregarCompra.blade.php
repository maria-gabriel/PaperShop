...
<div class="container" style="background-image: url(img/fondo.jpg); padding: 20px; width: 79%;">
   <div class="row justify-content-center">
        ...
        <div class="alert alert-success" role="alert">
            {{session('msj')}}</div>
        @endif
        <div class="card-header">{{ __('Ingrese los datos que se solicitan para una nueva compra') }}</div>
            <div style="padding: 20px;">
                <form action="{{ route('agregandoCompra') }}" class="newsletterForm" method="post" enctype="multipart/form-data">
                @csrf
